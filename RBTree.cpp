...
void RBTree::printTree(Node *root, std::string indent, bool last) {
...
RBTree::Node* RBTree::findParent(int num) {
...
RBTree::Node* RBTree::findSibling(Node *node) {
    if(!node -> parent)
        return nullptr;
    if(isOnLeft(node))
...
RBTree::Node* RBTree::findSuccessor(Node *node) {
    Node* successor;
    if(node) {
        if( node->rightChild)
...
RBTree::Node* RBTree::findUncle(Node *node) {
...
void RBTree::leftRotate(Node *higherNode) {
...
void RBTree::rightRotate(Node *higherNode) {
...
RBTree::Node* RBTree::getMinimumValue(Node *node) {
    while( node->leftChild ) node = node->leftChild;
    return node;
}

RBTree::Node* RBTree::BSTreplace(Node *x) {
...
RBTree::Node* RBTree::findRBT(int num) {
    Node * node = root;
    while(( node) && (node->data != num ) )
        if(num < node->data ) node = node->leftChild;
...
bool RBTree::isOnLeft(Node *node) { return node == node -> parent->leftChild; }

RBTree::Node::Node(bool isRed) {
    this -> isRed = isRed;
}

RBTree::Node::Node(int num, Node *parent) {
    this -> data = num;
    this -> parent = parent;
}
